...
**Central gallery of converted React landing pages (27 pages: Vite, Tailwind CSS 4, React Router, i18next).**
...
This project migrates the `Landing-Pages/` repository (vanilla JS, Bootstrap, jQuery, Tailwind
templates) into modern React pages. Each landing page becomes a real route
(`/landing-pages/<slug>`) rendered from JSX + hooks, assembled in a single React app.
...
- Each landing is **lazy-loaded** and rendered in a bare `LandingLayout` (no global chrome),
  with an RTL/Arabic handling where the source template is Arabic.
...
| **Styling** | Source CSS kept and scoped per page (some via `scope-css.mjs`), Tailwind 4 for Tailwind sources |
| **Icons** | `react-icons` everywhere except Dashboard, which keeps Font Awesome webfonts for CSS-glyph fidelity |
...
- **react-icons** + Font Awesome 6.2 webfonts (Dashboard)
...
├── components/
│   ├── common/            # Loading, ErrorBoundary, BackToGallery, PendingPage
│   └── <Landing>/         # One folder per landing page
│       ├── <Section>.jsx  # Each section = a mini-component
│       ├── data.js        # Content arrays (nav, features, grids, …)
│       ├── css/           # Original CSS3 (scoped where reset-heavy)
│       ├── images/        # Landing images (imported as assets)
│       └── webfonts/      # Font Awesome (Dashboard only)
├── config/
│   ├── i18n.js            # i18n configuration
│   └── landingPages.js    # Central registry: slug, title, category, screenshot, lazy component
├── hooks/                 # useCountdown, useTilt, useStickyNav, …
├── layouts/
...
1. Create `src/components/<Landing>/` with its section components, `data.js` and `css/`.
2. Create `src/pages/<Landing>Page.jsx` that imports the page CSS and assembles the sections.
